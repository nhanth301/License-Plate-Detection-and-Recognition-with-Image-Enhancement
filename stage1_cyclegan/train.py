...
# weights_init function remains the same
def weights_init(m):
...
def train(args):
    ...
    # Helper function to denormalize and prepare images for plotting
    def denormalize(img_tensor):
        img_tensor = img_tensor * 0.5 + 0.5  # Map [-1, 1] to [0, 1]
    ...
    for epoch in range(args.epochs):
        ...
        for i, (real_A, real_B) in progress_bar:
            ...
            # Set requires_grad for discriminators to False to save computation
            ...
            loss_cycle_A = criterion_cycle(recons_A, real_A) * 30.0
            recons_B = netG_AtoB(fake_A)
            loss_cycle_B = criterion_cycle(recons_B, real_B) * 30.0
